...
class MLP(nn.Module):
    ...
    def reset_parameters(self):
        for layer in self.lins:
            ...
            torch.nn.init.normal_(layer.bias, mean=0.0, std=1e-2)
    def forward(self, x):
...
class ConvLayer(MessagePassing):
    ...
    def forward(self, 
        ...
        #/print("    ConvLayer edge_attr:",edge_attr.shape,edge_attr.std())
        h0 = self.propagate(
        ...
        #/print("    ConvLayer h0:",h0.shape,h0.std())
        #/print("    ConvLayer self.alpha:",self.alpha.shape,self.alpha.mean())
        out = self.node_mlp(self.alpha.view(1, -1) * h0 + x_dst)
        #/print("    ConvLayer out:",out.shape,out.std())
        return out
